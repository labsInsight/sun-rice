<template>
  <div class="sun-swipe">
    <div :style="trackStyle" class="sun-swipe-track" @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd"
      @touchcancel="onTouchEnd" @transitionend="$emit('change', activeIndicator)">
      <slot />
    </div>
    <div v-if="showIndicators && count > 1" :class="['sun-swipe-indicators', { 'sun-swipe-vertical': vertical }]">
      <i v-for="index in count" :key="index" :class="['sun-swipe-indicator', { 'sun-swipe-vertical': vertical, 'sun-swipe-active': index - 1 === activeIndicator }]" />
    </div>
  </div>
</template>
...
@component-namespace sun {
  @component swipe {
    ...
    @descendent vertical {
      ...
      .sun-swipe-indicator:not(:last-child) {
        margin-bottom: $sun-swipe-indicator;
      }
    }
    ...
    @descendent indicator {
      ...
      width: $sun-swipe-indicator;
      height: $sun-swipe-indicator;

      &:not(:last-child) {
        margin-right: $sun-swipe-indicator;
        &.sun-swipe-vertical {
          margin-bottom: $sun-swipe-indicator;
        }
      }
